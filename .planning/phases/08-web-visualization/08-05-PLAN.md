...
- `new_observation` event: dispatch `memorite:new_observation` with the payload data. Both graph.js and timeline.js listen for this.
- `entity_updated` event: dispatch `memorite:entity_updated`. Graph.js updates/adds the node.
- `topic_shift` event: dispatch `memorite:topic_shift`. Timeline.js inserts a shift marker.
- `session_start` event: dispatch `memorite:session_start`. Timeline.js creates a new session card.
- `session_end` event: dispatch `memorite:session_end`. Timeline.js updates the session card.
