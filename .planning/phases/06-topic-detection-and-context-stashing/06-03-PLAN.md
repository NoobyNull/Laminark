...
must_haves:
  truths:
    ...
    - "User is notified that context was stashed with a message indicating /laminark:resume"
    - "User can manually stash current context via /laminark:stash slash command"
  ...
  artifacts:
    ...
    - path: "src/commands/stash.ts"
      provides: "/laminark:stash slash command handler"
...
<objective>
Wire the topic shift detector and stash manager into the hook pipeline so that automatic stashing occurs on detected topic shifts. Add user notification and the /laminark:stash manual stash command.
...
Output: TopicShiftHandler that runs on each PostToolUse, /laminark:stash command handler, notification message format.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    3. If shifted:
       ...
       e. Return { stashed: true, notification: "Topic shift detected. Previous context stashed: \"{topicLabel}\". Use /laminark:resume to return." }
    ...
    - Notification message includes topic label and /laminark:resume hint
...
<task type="auto">
  <name>Task 2: /laminark:stash slash command</name>
  <files>src/commands/stash.ts</files>
  <action>
    Create the /laminark:stash slash command handler (follow the pattern established by /laminark:remember and /laminark:recall from Phase 5):
    ...
    7. Return { success: true, message: "Context stashed: \"{topicLabel}\". Use /laminark:resume to return to it." }
  ...
  <done>/laminark:stash saves current session observations as a stash. Returns confirmation message. Registered in command router.</done>
</task>
...
- Notification message format includes topic label and /laminark:resume instruction
- /laminark:stash command manually creates stash from current session
...
- /laminark:stash manual command works (UI-04)
