...
3. Methods:
   - `start()`: Begin periodic curation on setInterval. Log "[laminark:curation] Agent started, interval: Nms"
   - `stop()`: Clear interval, set running = false. Log "[laminark:curation] Agent stopped"
...
4. `runCuration(db: Database): Promise<CurationReport>` (standalone function, also usable without class):
   ...
   **Step 5: Low-value pruning**
   ...
   - Log summary: "[laminark:curation] Cycle complete: N merged, N deduped, N flagged stale, N pruned"
