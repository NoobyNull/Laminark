...
must_haves:
  truths:
    - "User can type /laminark:remember followed by text and it triggers the save_memory MCP tool to persist a memory"
    - "User can type /laminark:recall followed by a description and it triggers the search MCP tool to find relevant memories"
    - "/laminark:remember includes the user's text as the memory content with appropriate context"
    - "/laminark:recall displays search results in a readable format with relevance indicators"
  artifacts:
    - path: "commands/remember.md"
      provides: "Slash command definition for /laminark:remember"
      contains: "save_memory"
    - path: "commands/recall.md"
      provides: "Slash command definition for /laminark:recall"
      contains: "search"
  key_links:
...
<objective>
/laminark:remember and /laminark:recall slash command implementations.

Purpose: Slash commands give users explicit, discoverable control over their memory system. /laminark:remember lets users save important context with a simple command. /laminark:recall lets users search their memories by description. These commands bridge the gap between automatic observation capture (Phase 3) and intentional memory management -- users can explicitly tell the system "this is important, remember it" or "find what I know about X."
...
<task type="auto">
  <name>Task 1: Create /laminark:remember slash command</name>
  <files>
    commands/remember.md
  </files>
  <action>
    ...
    2. Create `commands/remember.md` with the following content. This file IS the slash command -- Claude Code reads it as instructions when the user types `/laminark:remember`.
  ...
  <done>
    /laminark:remember slash command is defined in commands/remember.md. When a user types the command followed by text, Claude calls the save_memory MCP tool with the user's text and source "slash:remember". The command includes usage instructions, examples, and handles the edge case of no text provided.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create /laminark:recall slash command</name>
  <files>
    commands/recall.md
  </files>
  <action>
...
<success_criteria>
- UI-02 is satisfied: /laminark:remember allows users to explicitly save a memory with context
- UI-03 is satisfied: /laminark:recall allows users to search memories by description and see relevant results
