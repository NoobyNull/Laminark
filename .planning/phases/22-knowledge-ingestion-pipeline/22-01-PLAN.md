...
1. Export `IngestionStats` interface with exactly 3 fields (no `sectionsUpdated` field -- the soft-delete+re-create strategy does not distinguish updates from creates. The RESEARCH.md Example 2 includes a 4th `sectionsUpdated` field; ignore it.):
   ```typescript
