...
- Log rejections via debug('hook', 'Observation rejected', { tool, reason })
  ...
  <verify>
Run `npm test -- admission-filter` and `npm run check`.
  </verify>
  <done>
All noise pattern tests pass. Write/Edit tool observations are never rejected based on content. Bash noise (build output, package install, linter spam, empty output) is correctly rejected. RED-GREEN-REFACTOR commits produced.
  </done>
</task>
...
5. `redactSensitiveContent(text: string, filePath?: string): string | null`:
  ...
  <verify>
Run `npm test -- privacy-filter` and `npm run check`.
  </verify>
  <done>
All privacy pattern tests pass. API keys, JWTs, connection strings, private keys, and env variable values are correctly redacted. Excluded files (.env, credentials, secrets, .pem, .key, id_rsa) return null. User-configured additional patterns are applied. RED-GREEN-REFACTOR commits produced.
  </done>
</task>
