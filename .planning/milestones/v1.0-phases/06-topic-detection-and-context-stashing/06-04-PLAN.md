...
must_haves:
  truths:
    - "User can type /laminark:resume and see a list of stashed context threads"
  ...
  artifacts:
    - path: "src/commands/resume.ts"
      provides: "/laminark:resume slash command handler"
...
<objective>
Build the /laminark:resume slash command and the topic_context MCP tool that powers "where was I?" queries. These are the user's path back to stashed context threads.
...
Output: /laminark:resume command handler, topic_context MCP tool, tests for both.
</objective>
...
<task type="auto">
  <name>Task 1: /laminark:resume slash command</name>
  <files>src/commands/resume.ts, src/commands/__tests__/resume.test.ts</files>
  <action>
    Create /laminark:resume command handler (follow Phase 5 slash command patterns):
    ...
    1. No stashId provided (list mode):
       ...
       - Include instruction: "Use /laminark:resume {id} to restore a thread."
    ...
    Register in slash command router alongside /laminark:stash.
  ...
  <done>/laminark:resume lists stashed threads in list mode, restores context in resume mode. Status updated on resume. All tests pass.</done>
</task>
...
<verification>
- /laminark:resume lists stashes when no ID given
- /laminark:resume restores context and updates status when ID given
...
<success_criteria>
- /laminark:resume works in both list and resume modes (CTX-05, UI-05)
